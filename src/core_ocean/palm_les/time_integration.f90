...
!------------------------------------------------------------------------------!
 SUBROUTINE time_integration
    ...
    USE arrays_3d,                                                             &
        ONLY:  alpha_T, beta_S, dzu, ddzu, ddzw, dzw, ddzuw, dd2zu, e, e_p,    &
               hyp, kh, km, p, prho, pt_init, pt, pt_p, rdf, rdf_sc,           &
               rho_ocean, sa_init, sa, sa_p, sgs_diss, solar3d, tend,          &
               te_m, tpt_m, tsa_m, tu_m, tv_m, tw_m,                           &
#ifdef __SP
               pt_LS_forcing, sa_LS_forcing, u_LS_forcing, v_LS_forcing,       &
#endif
               u, u_p, v, v_p, u_stk, v_stk, u_init, v_init,                   &
               u_stk_zw, v_stk_zw, w, w_p, zw

    USE control_parameters,                                                    &
        ONLY:  average_count_3d, averaging_interval, averaging_interval_pr,    &
               call_psolver_at_all_substeps,                                   &
               create_disturbances, dopr_n, current_timestep_number,           &
               disturbance_created, do_sum, dp_smooth_factor, dpdxy,           &
               dt_3d, dt_averaging_input, dt_averaging_input_pr,               &
               dt_data_output_av, dt_disturb, dt_do3d, dt_dopr,                &
               dt_run_control, end_time, g, intermediate_timestep_count,       &
               intermediate_timestep_count_max, mid,                           &
               nr_timesteps_this_run,                                          &
               simulated_time, simulated_time_chr,                             &
               skip_time_do3d, skip_time_dopr,                                 &
               skip_time_data_output_av, stokes_force,                         &
               stop_dt, terminate_run,                                         &
               time_do3d, time_dopr, time_dopr_av,                             &
               time_dopr_listing, time_do_av, time_do_sla,                     &
               time_disturb, time_run_control, time_since_reference_point, tsc
    ...
    USE indices,                                                               &
        ONLY:  nbgp, nx, nxl, nxlg, nxr, nxrg, nyn, nyng, nys, nysg, nzb, nzt, &
               advc_flags_1, advc_flags_2, ngp_2dh
    ...
    USE statistics,                                                            &
        ONLY:  flow_statistics_called, hom, hom_sum, sums, weight_substep,     &
               sums_us2_ws_l, sums_vs2_ws_l, sums_ws2_ws_l, sums_wsus_ws_l,    &
               sums_wsvs_ws_l, sums_wspts_ws_l, sums_wssas_ws_l,               &
               sums_usvs_ws_l, sums_vsus_ws_l

    USE turbulence_closure_mod,                                                &
        ONLY:  tcm_diffusivities, l_grid, l_wall
...
!$acc      copyin( hom ) &
#ifdef __SP
!$acc      copyin( pt_LS_forcing, sa_LS_forcing ) &
!$acc      copyin( u_LS_forcing, v_LS_forcing ) &
#endif
!$acc      create( hom_sum ) &
!$acc      create( sums_wsus_ws_l ) &
!$acc      create( sums_wsvs_ws_l ) &
!$acc      create( sums_usvs_ws_l ) &
!$acc      create( sums_vsus_ws_l ) &
!$acc      create( sums_us2_ws_l ) &
!$acc      create( sums_vs2_ws_l ) &
!$acc      create( sums_ws2_ws_l ) &
!$acc      create( sums_wspts_ws_l ) &
!$acc      create( sums_wssas_ws_l ) &
!$acc      create( sgs_diss ) &
!$acc      create( sums )
...
!--       on the timestep scheme
          ...
          IF ( intermediate_timestep_count == 1 )  CALL ws_statistics
!

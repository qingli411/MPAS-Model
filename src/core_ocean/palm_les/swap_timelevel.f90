...
!------------------------------------------------------------------------------!
 SUBROUTINE swap_timelevel
    ...
    USE arrays_3d,                                                             &
        ONLY:  e, e_p, pt, pt_p, sa, sa_p, u, u_p, v, v_p, w, w_p
#else
    USE arrays_3d,                                                             &
        ONLY:  e, e_1, e_2, e_p, pt, pt_1, pt_2, pt_p, sa, sa_1, sa_2, sa_p,   &
...
!-- Swap of variables
    ...
    !$acc parallel present( u, v, w, pt, sa, e ) &
    !$acc present( u_p, v_p, w_p, pt_p, sa_p, e_p )
    !$acc loop collapse(3)
    DO  i = nxlg, nxrg
       DO  j = nysg, nyng
          DO  k = nzb, nzt+1

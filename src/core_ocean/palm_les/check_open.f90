...
SUBROUTINE check_open( file_id )
    ...
    USE control_parameters,                                                    &
        ONLY:  message_string, mid, nz_do3d, openfile,                         &
    ...
    USE netcdf_interface,                                                      &
        ONLY:  id_set_pr, id_set_3d, nc_stat, netcdf_create_file,              &
               netcdf_data_format, netcdf_define_header,                       &
...
!   run id
    IF ( run_id /= -9999 ) THEN
       WRITE (run_id_char,'(''_'',I7.7)')  run_id
    ELSE
       run_id_char = ''
    ENDIF
!
...
!-- Open relevant files
    SELECT CASE ( file_id )
       ...
       CASE ( 11 )
          ...
          OPEN ( 11, FILE='PARIN', FORM='FORMATTED',    &
                     STATUS='OLD' )

       CASE ( 13 )

          IF ( myid_char == '' )  THEN
             OPEN ( 13, FILE='BININ'//TRIM(run_id_char)//myid_char,        &
...
!--          only this file contains the global variables
             IF ( .NOT. openfile(file_id)%opened_before )  THEN
                OPEN ( 13, FILE='BININ'//TRIM(run_id_char)//'/_000000',    &
                           FORM='UNFORMATTED', STATUS='OLD' )
             ELSE
                OPEN ( 13, FILE='BININ'//TRIM(run_id_char)//'/'//          &
       ...
       CASE ( 14 )
          ...
          IF ( myid_char == '' )  THEN
             OPEN ( 14, FILE='BINOUT'//TRIM(run_id_char)//myid_char,       &
                        FORM='UNFORMATTED', POSITION='APPEND' )
          ELSE
             IF ( myid == 0  .AND. .NOT. openfile(file_id)%opened_before )  THEN
                CALL local_system( 'mkdir  BINOUT' )
             ENDIF
...
!--          directory created by PE0 can open their file
       ...
       CASE ( 15 )
          ...
          OPEN ( 15, FILE='RUN_CONTROL'//TRIM(run_id_char),                &
                     FORM='FORMATTED' )
       ...
       CASE ( 18 )
          ...
          if( .not. ISOPEN) THEN
             OPEN ( 18, FILE='CPU_MEASURES'//TRIM(run_id_char),               &
                     FORM='FORMATTED' )
          endif

       CASE ( 19 )

          OPEN ( 19, FILE='HEADER'//TRIM(run_id_char), FORM='FORMATTED' )

       CASE ( 20 )

          IF ( myid == 0  .AND. .NOT. openfile(file_id)%opened_before )  THEN
             CALL local_system( 'mkdir  DATA_LOG' )
          ENDIF
          IF ( myid_char == '' )  THEN
             OPEN ( 20, FILE='DATA_LOG'//TRIM(run_id_char)//'/_000000',    &
                        FORM='UNFORMATTED', POSITION='APPEND' )
          ELSE
#if defined( __parallel )
!
...
!--          directory created by PE0 can open their file
             ...
             ioerr = 1
             DO WHILE ( ioerr /= 0 )
                OPEN ( 20, FILE='DATA_LOG'//TRIM(run_id_char)//'/'//       &
       ...
       CASE ( 30 )
          ...
          OPEN ( 30, FILE='PLOT3D_DATA'//TRIM(run_id_char)//myid_char,     &
                     FORM='UNFORMATTED' )
!
...
!--    Progress file that is used by the PALM watchdog
       CASE ( 117 )
          ...
          OPEN ( 117, FILE='PROGRESS'//TRIM(run_id_char),                  &
...
!
!--       Set filename
          filename = 'DATA_1D_PR_NETCDF'//TRIM(run_id_char)
...
!--       Set filename depending on unit number
          IF ( file_id == 106 )  THEN
             filename = 'DATA_3D_NETCDF'//TRIM(run_id_char)
             av = 0
          ELSE
             filename = 'DATA_3D_AV_NETCDF'//TRIM(run_id_char)
             av = 1
          ENDIF
!
...
!--          combine_plot_fields that nothing is to do.
             IF ( myid == 0  .AND.  netcdf_data_format > 4 )  THEN
                OPEN( 99, FILE='NO_COMBINE_PLOT_FIELDS_3D'//TRIM(run_id_char) )

...
!------------------------------------------------------------------------------!
 SUBROUTINE pres
    ...
    USE arrays_3d,                                                             &
        ONLY:  d, ddzu, ddzw, p, tend, u, v, w

    USE control_parameters,                                                    &
        ONLY:  dt_3d, ibc_p_b, ibc_p_t,                                        &
    ...
    USE indices,                                                               &
        ONLY:  nbgp, ngp_2dh, nx, nxl, nxlg, nxr, nxrg,                        &
               ny, nys, nysg, nyn, nyng, nzb, nzt
    ...
    USE statistics,                                                            &
        ONLY:  weight_pres, weight_substep
...
!--    If pres is called before initial time step
    ...
    THEN
       ...
       !$acc parallel present( w_l, ngp_2dh )
       !$acc loop
       DO  k = 1, nzt
          w_l(k) = w_l(k) / ngp_2dh
       ENDDO
       !$acc end parallel
       !$acc parallel present( w_l, w )
       !$acc loop collapse(2)
       DO  i = nxlg, nxrg
          DO  j = nysg, nyng
             !$acc loop seq
...
!-- Compute the divergence of the provisional velocity field.
    ...
    !$acc parallel present( d, u, v, w, ddzw )
    !$acc loop
    DO  i = nxl, nxr
       !$acc loop
       DO  j = nys, nyn
...
!--    Neumann (dp/dz = 0).
       ...
       !$acc parallel loop present( tend )
       DO  i = nxlg, nxrg
          DO  j = nysg, nyng
...
!-- Ghost layers are added in the output routines (except sor-method: see below)
    ...
    !$OMP PARALLEL PRIVATE (i,j,k)
    !$OMP DO
    !$acc parallel present( u, v, w, tend, ddzu )
    !$acc loop
    DO  i = nxl, nxr
       !$acc loop
       DO  j = nys, nyn

...
!------------------------------------------------------------------------------!
 SUBROUTINE parin
    ...
    NAMELIST /inipar/  bc_e_b, bc_p_b, bc_p_t, bc_pt_b, bc_pt_t,               &
             bc_sa_b, bc_sa_t, bc_uv_b, bc_uv_t, call_psolver_at_all_substeps, &
             cfl_factor, collective_wait, day_of_year_init, dp_external,       &
             dp_level_b, dp_smooth, dpdxy, dt, dx, dy, dz, dz_max,             &
             dz_stretch_factor, dz_stretch_level, dz_stretch_level_start,      &
             dz_stretch_level_end, d_stk, ensemble_member_nr, e_init, e_min,   &
             idealized_diurnal, ideal_solar_heatflux, ideal_solar_division,    &
             ...
             scalar_rayleigh_damping, stokes_force, stokes_drift_method,       &
             surface_pressure, time_utc_init, top_heatflux,                    &
             top_momentumflux_u, top_momentumflux_v, top_salinityflux,         &
             transpose_compute_overlap, turbulence_closure, u0_stk,            &
             uv_heights, u_profile, v0_stk, v_profile, wall_adjustment,        &
             wave_age, wind_dir, wind_speed

    NAMELIST /initialization_parameters/  bc_e_b, bc_p_b, bc_p_t,              &
             bc_pt_b, bc_pt_t, bc_sa_b, bc_sa_t, bc_uv_b, bc_uv_t,             &
             call_psolver_at_all_substeps, cfl_factor, collective_wait,        &
             day_of_year_init, dp_external, dp_level_b, dp_smooth, dpdxy,      &
             dt, dx, dy, dz, dz_max,                                           &
             dz_stretch_factor, dz_stretch_level, dz_stretch_level_start,      &
             dz_stretch_level_end, d_stk, ensemble_member_nr, e_init, e_min,   &
             idealized_diurnal, ideal_solar_heatflux, ideal_solar_division,    &
    ...
    NAMELIST /d3par/  averaging_interval, averaging_interval_pr,               &
             ...
             disturbance_level_t, do3d_at_begin,                &
             dt, dt_averaging_input, dt_averaging_input_pr,                    &
             dt_data_output, dt_data_output_av, dt_disturb,       &
             dt_dopr, dt_dopr_listing,         &
             dt_do3d, dt_max, dt_restart,              &
             dt_run_control,end_time, force_print_header,        &
             netcdf_data_format, netcdf_deflate,     &
             npex, npey, nz_do3d,                          &
             profile_columns, profile_rows,     &
             restart_time,                  &
             skip_time_data_output, skip_time_data_output_av, skip_time_dopr,  &
             skip_time_do3d, synchronous_exchange,           &
    ...
    NAMELIST /envpar/  host,                &
                       ...
                       revision, run_identifier, tasks_per_node,     &
                       write_binary
...
!--                  also check the allocate statement in routine rrd_global
             ALLOCATE( pt_init(0:nz+1), sa_init(0:nz+1),           &

...
!------------------------------------------------------------------------------!
 MODULE fft_xy
    ...
    USE control_parameters,                                                    &
        ONLY:  init_fft, fft_method, message_string
    ...
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  ifax_x  !<
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  ifax_y  !<
    ...
    REAL(wp), DIMENSION(:), ALLOCATABLE ::  trigs_x  !< 
    REAL(wp), DIMENSION(:), ALLOCATABLE ::  trigs_y  !<

#ifdef __GPU
    
    INTEGER :: nx_cC, ny_cC

    INTEGER :: batch1, batch2

    COMPLEX(wp), DEVICE, DIMENSION(:,:,:), ALLOCATABLE ::       &
            x_out_dev, y_out_dev
    INTEGER :: plan_xf_dev, plan_xi_dev, plan_yf_dev, plan_yi_dev
       ...
       if ( forward_fft )  THEN
          ...
          !$acc loop collapse(2)
             DO  k = nzb_x, nzt_x
                ...
                !$acc loop
                   DO  i = 0, (nx+1)/2
                   ar(i,j,k) = REAL( x_out_dev(i,j,k), KIND=wp ) / ( nx+1 )
                   ENDDO
                !$acc loop
                   DO  i = 1, (nx+1)/2 - 1
                   ar(nx+1-i,j,k) = AIMAG( x_out_dev(i,j,k) ) / ( nx+1 )
                   ENDDO
                ENDDO
             ENDDO
          !$acc end parallel
          ...
          !$acc parallel deviceptr(ar, x_out_dev)
          !$acc loop collapse(2)
             DO  k = nzb_x, nzt_x
                DO  j = nys_x, nyn_x

                x_out_dev(0,j,k) = CMPLX( ar(0,j,k), 0.0_wp, KIND=wp )
                !$acc loop
                   DO  i = 1, (nx+1)/2 - 1
                    x_out_dev(i,j,k) = CMPLX( ar(i,j,k), ar(nx+1-i,j,k), KIND=wp )
                   ENDDO
                    x_out_dev((nx+1)/2,j,k) = CMPLX( ar((nx+1)/2,j,k), 0.0_wp,       &
                                              KIND=wp )
       ...
       ENDIF
          ...
          DO  k = nzb_y, nzt_y
             DO  i = nxl_y, nxr_y
                ...
                !$acc loop
                DO  j = 0, (ny+1)/2
                   ar(j,i,k) = REAL( y_out_dev(j,i,k), KIND=wp ) / (ny+1)
                   ENDDO
                !$acc loop
                DO  j = 1, (ny+1)/2 - 1
                   ar(ny+1-j,i,k) = AIMAG( y_out_dev(j,i,k) ) / (ny+1)
                   ENDDO
          ...
          DO  k = nzb_y, nzt_y
             DO  i = nxl_y, nxr_y
                ...
                y_out_dev(0,i,k) = CMPLX( ar(0,i,k), 0.0_wp, KIND=wp )
                !$acc loop
                DO  j = 1, (ny+1)/2 - 1
                   y_out_dev(j,i,k) = CMPLX( ar(j,i,k), ar(ny+1-j,i,k),       &
                                     KIND=wp )
                   ENDDO
                y_out_dev((ny+1)/2,i,k) = CMPLX( ar((ny+1)/2,i,k), 0.0_wp,       &
                                         KIND=wp )
                   ENDDO
                   ENDDO
          !$acc end parallel

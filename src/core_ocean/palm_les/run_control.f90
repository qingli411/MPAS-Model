...
!------------------------------------------------------------------------------!
 SUBROUTINE run_control
    ...
    USE control_parameters,                                                    &
        ONLY:  current_timestep_number, disturbance_created, dt_3d,            &
    ...
    USE statistics,                                                            &
        ONLY:  flow_statistics_called, pr_palm, u_max, u_max_ijk, v_max,  &
...
!--    If required, set disturbance flag
       ...
       WRITE ( 15, 101 )  runnr, current_timestep_number, simulated_time_chr,  &
                          ...
                          v_max, disturb_chr, w_max, u_max_ijk(1:3),           &
...
100 FORMAT (///'Run-control output:'/ &
              &'------------------'// &
          &'RUN  ITER. HH:MM:SS.SS    DT(E)     UMAX     VMAX     WMAX', &
          &'      UMAX(KJI)    VMAX(KJI)    WMAX(KJI)'/                  &
          &'----------------------------------------------------------', &
          &'-----------------------------------------')

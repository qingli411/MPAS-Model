...
!--    Compute Stokes forces for the three velocity components
       !$acc data present( tend, ddzu ) &
       !$acc present( u_stk, v_stk ) &
       !$acc present( u_stk_zw, v_stk_zw ) &
       !$acc present( u, v, w )
...
!--                   Stokes-Coriolis force
                      tend(k,j,i) = tend(k,j,i) + f * v_stk(k)
!
!--                   Stokes-vortex force
...
!
!--                   Stokes-Coriolis force
                      tend(k,j,i) = tend(k,j,i) - f * u_stk(k)
!
!--                   Stokes-vortex force
...
!
!--                   Stokes-Coriolis force
                      tend(k,j,i) = tend(k,j,i) + fs * u_stk_zw(k)
!
!--                   Stokes-vortex force
...
!--    Compute Stokes-advection term for the tracer equation
       !$acc parallel present( tend, u_stk, v_stk ) &
       !$acc present( sk )
       !$acc loop
       DO  i = nxl, nxr
          !$acc loop
...
!--    Update perturbation pressure
       !$acc parallel present( p, u, v, u_stk, v_stk )
       !$acc loop
       DO  i = nxl, nxr
          !$acc loop
          DO  j = nys, nyn
